#ifndef SPAN_SRC_SPAN_THIRD_PARTY_SLIMSIG_DETAIL_SIGNAL_BASE_HH_
#define SPAN_SRC_SPAN_THIRD_PARTY_SLIMSIG_DETAIL_SIGNAL_BASE_HH_
...
namespace slimsig {
  ...
  template <class Handler, class SignalTraits, class Allocator>
  void swap(signal<Handler, SignalTraits, Allocator>& lhs,
            signal<Handler, SignalTraits, Allocator>& rhs);

  namespace detail {
    template <class Container, class Callback>
    inline void each(const Container& container,
                     typename Container::size_type begin,
                     typename Container::size_type end, const Callback& fn) {
      for (; begin != end; begin++) {
        fn(container[begin]);
      }
    }
    template <class Container, class Callback>
    inline void each_n(const Container& container,
                       typename Container::size_type begin,
                       typename Container::size_type count, const Callback& fn) {
  ...
  class signal_base<SignalTraits, Allocator, R(Args...)> {
   ...
   public:
    ...
    using list_allocator_type = typename std::allocator_traits<
        Allocator>::template rebind_traits<slot>::allocator_type;
   ...
   public:
    ...
    signal_base(signal_base&& other) { this->swap(other); }
    ...
    inline connection connect(
        std::shared_ptr<signal<R(Args...), TP, Alloc>> signal) {
    ...
    inline size_type slot_count() const { return m_size; }
    inline size_type max_size() const {
      return std::min(std::numeric_limits<slot_id>::max(), pending.max_size());
    }
    inline size_type remaining_slots() const { return max_size() - last_id; }

    size_type max_depth() const { return std::numeric_limits<unsigned>::max(); }
    size_type get_depth() const { return m_depth; }
    bool is_running() const { return m_depth > 0; }
   ...
   private:
    struct emit_scope {
      signal_base& signal;
      emit_scope(signal_base& context) : signal(context) { signal.m_depth++; }
      emit_scope() = delete;
      ...
      ~emit_scope() {
        ...
        // if we completed iteration (depth = 0) collapse all the levels into the
        // head list
        if (depth == 0) {
          ...
          if (m_size != pending.size()) {
            ...
            pending.erase(
                remove_if(pending.begin(), pending.end(), &is_disconnected),
                pending.end());
          }
          signal.m_offset = 0;
    ...
    inline bool connected(slot_id index) {
      ...
      auto slot = lower_bound(pending.cbegin() + m_offset, end, index,
                              [](const_slot_reference slot,
                                 const slot_id& index) { return slot < index; });
    ...
    inline void disconnect(slot_id index) {
      ...
      auto slot = lower_bound(
          pending.begin() + m_offset, end, index,
          [](slot_reference slot, const slot_id& index) { return slot < index; });
    ...
    [[gnu::always_inline]] inline slot_id prepare_connection() {
      ...
      assert((last_id < std::numeric_limits<slot_id>::max() - 1) &&
             "All available slot ids for this signal have been exhausted. This "
             "may be a sign you are misusing signals");
      return last_id++;
    };
  ...
  void swap(signal<Handler, ThreadPolicy, Allocator>& lhs,
            signal<Handler, ThreadPolicy, Allocator>& rhs) {
    using std::swap;
    lhs.swap(rhs);
  }
}  // namespace slimsig

#endif  // SPAN_SRC_SPAN_THIRD_PARTY_SLIMSIG_DETAIL_SIGNAL_BASE_HH_
